...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let rawMarkdownForDownload = ''; // Store raw markdown from AI
    ...
    if(generateProposalButton) generateProposalButton.addEventListener('click', async function() {
        ...
        // Implicitly save API key if it looks valid and is different from stored one
        if ((apiKey.startsWith('sk-') || apiKey.startsWith('sk-proj-')) && apiKey !== userProvidedApiKey) {
            ...
            // Optionally alert the user: alert('API Key updated and saved for this session.');
        ...
        try {
            ...
            const response = await fetch(aiApiEndpoint, {
                ...
                headers: { 
                    ...
                    'Authorization': `Bearer ${userProvidedApiKey}` // Use the API key from state/input
                },
        ...
        const htmlToDownload = `
            ...
            <head>
                ...
                <style>
                    ...
                    ul, ol { margin-left: 20px; margin-bottom: 12px; padding-left: 20px; }
                    li { margin-bottom: 6px; }
                    strong { font-weight: bold; }
                    em { font-style: italic; }
                    hr { border: 0; height: 1px; background: #cccccc; margin: 25px 0; }
